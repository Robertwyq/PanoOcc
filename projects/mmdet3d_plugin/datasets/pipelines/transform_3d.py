...
class ResizeCropFlipImage(object):
    ...
    def __call__(self, results):
        ...
        for i in range(N):
            ...
            if 'gt_semantic_seg' in results.keys():
                mask = Image.fromarray(np.uint8(masks[i]))
                mask, _ = self._img_transform(
                    mask,
                    resize=resize,
                    resize_dims=resize_dims,
                    crop=crop,
            ...
            if 'gt_semantic_seg' in results.keys():
                new_masks.append(np.array(mask).astype(np.float32))
            results['cam_intrinsic'][i][:3, :3] = ida_mat @ results['cam_intrinsic'][i][:3, :3]

        results["img"] = new_imgs
        if 'gt_semantic_seg' in results.keys():
            results["gt_semantic_seg"] = new_masks
...
class CustomCollect3D(object):
    ...
    def __init__(self,
                 ...
                 meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img','ego2lidar',
                            ...
                            'can_bus', "ego2global_transformation", "lidar2ego_transformation"
                            )):
        self.keys = keys
...
@PIPELINES.register_module()
class RandomMultiScaleImageMultiViewImage(object):
    """Random scale the image
    Args:
        scales
    """
    ...
    def __call__(self, results, seed=None):
        ...
        # add mask
        if 'gt_semantic_seg' in results.keys():
            results['gt_semantic_seg'] = [mmcv.imresize(sem, (x_size[idx], y_size[idx]), return_scale=False) for idx, sem in enumerate(results['gt_semantic_seg'])]
        lidar2img = [scale_factor @ l2i for l2i in results['lidar2img']]
        cam_intrinsics_new = [scale_factor @ cam_intrinsic for cam_intrinsic in results['cam_intrinsic']]
