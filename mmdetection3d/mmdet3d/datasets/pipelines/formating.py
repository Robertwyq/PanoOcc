...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        ...
        if 'gt_semantic_seg' in results:
            if isinstance(results['gt_semantic_seg'], list):
                semantic = np.ascontiguousarray(np.stack(results['gt_semantic_seg'], axis=0))
                results['gt_semantic_seg'] = DC(to_tensor(semantic), stack=True)
            else:
